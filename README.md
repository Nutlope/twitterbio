...
This project uses both [Kimi K2.5](https://api.together.xyz/playground/chat/deepseek-ai/deepseek-llm-7b-chat) and [Opus 4.6](https://api.together.xyz/playground/chat/gpt-4o) with streaming to generate a Twitter bio. It constructs a prompt based on the form and user input, sends it to the [Together.ai](https://togetherai.link/) API, then streams the response back to the application. Kimi K2.5 has thinking disabled for fast direct responses. Opus 4.6 is a reasoning model that thinks before responding, shown with a "Thinking..." indicator.
